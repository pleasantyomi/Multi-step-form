...
const Body = () => {
    ...
    const[option, setOption] = useState([
        {number: '1', title: 'STEP 1', subtitle: 'YOUR INFO', Page: 0, id: 1},
            ...
            <div className="lg:absolute lg:top-[50%] lg:-translate-y-1/2 lg:left-[50%] lg:-translate-x-1/2 lg:bg-white lg:flex lg:w-8/12 h-[80vh] lg:rounded-xl  lg:shadow-sm lg:p-3">
                <div  className=" lg:bg-desktopbg bg-mobilebg flex-shrink-0 flex-nowrap lg:w-4/12 w-full h-full bg-contain   bg-no-repeat lg:pl-5 lg:mr-6 lg:pt-10">
                    <div className="lg:block flex justify-center gap-5 lg:pt-0 pt-10">
                       {option.map((options) => (
                       <div key={options.id}>                              
                            <button onClick={PageDisplay} className="flex items-center lg:mb-5">
                                    <div className="relative bg-transparent border border-white w-9 h-9 rounded-full mr-3">
                                     <p className="absolute top-[50%] -translate-y-1/2 left-[50%] -translate-x-1/2 text-white">{options.number}</p>
                                    </div>
                                    <div className="hidden lg:grid text-white text-left">
                                       <span className="text-xs font-thin text-light-gray">{options.title}</span>
                                       <span className="font-semibold">{options.subtitle}</span>
                                    </div>
                            </button>
                        </div>
                        ))}
                ...
                <div className="hidden lg:block lg:relative lg:ml-6 lg:h-full lg:w-6/12">
                    ...
                    <div className="flex justify-between items-center">
                       <button onClick={prevPage} disabled={Page == 0} hidden={Page === 4} className="absolute bottom-0 mb-5 font-semibold text-cool-gray">{Page === 0 ? "" : "Go Back"}</button>
                       <button onClick={nextPage} hidden={Page === 4} className="absolute bottom-0 right-0 mb-5 bg-marine-blue font-semibold text-white px-4 py-2 rounded-md">{Page === PageTtiles.length - 1 ? "Confirm" : "Next Step"}</button>
               ...
               <div className="lg:hidden w-11/12 ml-auto mr-auto">
                    ...
                    <div className="flex justify-between items-center">
                       <button onClick={prevPage} disabled={Page == 0} hidden={Page === 4} className="font-semibold text-cool-gray">{Page === 0 ? "" : "Go Back"}</button>
                       <button onClick={nextPage} hidden={Page === 4} className=" bg-marine-blue font-semibold text-white px-4 py-2 rounded-md">{Page === PageTtiles.length - 1 ? "Confirm" : "Next Step"}</button>
                    </div>
                </div>
